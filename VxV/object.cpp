...
void loadingObject(const std::string& filename, tinyobj::attrib_t& attributes, std::vector<tinyobj::shape_t>& shapes, std::vector<tinyobj::material_t>& materials, GLuint& vertexbuffer, GLuint& indexbuffer)
{
	std::string warnings;
	std::string errors;
	...
	for (size_t s = 0; s < shapes.size(); s++)
	{
		...
		for (size_t f = 0; f < mesh.indices.size(); f++)
		{
			...
			Vertex vertex = {};
			vertex.position = {attributes.vertices[3 * index.vertex_index + 0], attributes.vertices[3 * index.vertex_index + 1], attributes.vertices[3 * index.vertex_index + 2]};
			vertex.normal = {attributes.normals[3 * index.normal_index + 0], attributes.normals[3 * index.normal_index + 1], attributes.normals[3 * index.normal_index + 2]};
			vertex.texCoord = {attributes.texcoords[2 * index.texcoord_index + 0], attributes.texcoords[2 * index.texcoord_index + 1]};
...
void loadObjAndTextures(const std::string& filename, tinyobj::attrib_t& attributes, std::vector<tinyobj::shape_t>& shapes, std::vector<tinyobj::material_t>& materials, GLuint& vertexbuffer, GLuint& indexbuffer, std::vector<GLuint>& textureIDs)
{
	loadingObject(filename, attributes, shapes, materials, vertexbuffer, indexbuffer);
	loadTextures(materials, textureIDs);
}

void batchingObj(std::vector<tinyobj::shape_t>& shapes, tinyobj::attrib_t& attributes, std::vector<GLuint>& textureIDs, std::unordered_map<std::pair<GLuint, int>, std::vector<Vertex>, pair_hash>& vertexBuffers, std::unordered_map<std::pair<GLuint, int>, GLuint, pair_hash>& vertexBufferIDs)
{
	for (size_t s = 0; s < shapes.size(); s++)
	{
		tinyobj::mesh_t& mesh = shapes[s].mesh;
...
void loadObjAndBatching(const std::string& filename, tinyobj::attrib_t& attributes, std::vector<tinyobj::shape_t>& shapes, std::vector<tinyobj::material_t>& materials, GLuint& vertexbuffer, GLuint& indexbuffer, std::vector<GLuint>& textureIDs, std::unordered_map<std::pair<GLuint, int>, std::vector<Vertex>, pair_hash>& vertexBuffers, std::unordered_map<std::pair<GLuint, int>, GLuint, pair_hash>& vertexBufferIDs)
{
...
void drawObjects(std::vector<Object>& objects, GLuint TextureID, GLuint MaterialAmbientColorID, GLuint MaterialDiffuseColorID, GLuint MaterialSpecularColorID)
{
